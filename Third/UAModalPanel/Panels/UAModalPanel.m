...
#define DEFAULT_MARGINH             DEFAULT_MARGIN
#define DEFAULT_MARGINV             DEFAULT_MARGIN
#define DEFAULT_CONTENT_HEIGHT      0.0f
...
@synthesize outerMargin;
@synthesize outerMarginH;
@synthesize outerMarginV;
...
- (id)initWithFrame:(CGRect)frame
{
	...
	if (self != nil)
    {
        ...
        outerMarginH = DEFAULT_MARGINH;
        outerMarginV = DEFAULT_MARGINV;
...
//					  self.frame.size.height - 2*self.outerMargin);
	return CGRectMake(self.outerMarginH + self.frame.origin.x,
					  self.outerMarginV + self.frame.origin.y,
					  self.frame.size.width - 2*self.outerMarginH,
                      self.contentHeight != DEFAULT_CONTENT_HEIGHT ? self.contentHeight:self.frame.size.height - 2*self.outerMarginV);
}
...
- (void)hide
{
    ...
    {
        self.alpha = 0;
        if (_startEndPoint.x != CGPointZero.x || _startEndPoint.y != CGPointZero.y)
        {
            self.contentContainer.center = _startEndPoint;
        }
